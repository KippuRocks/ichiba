...
describe("scan", () => {
  ...
  it("finds terms in string literals, template text, JSX text and JSX attributes", () => {
    project({
      "src/Screen.tsx": [
        ...
        "const b = `Trustless \u0024{a}`;",
        "export const Screen = () => (",
        '  <Link aria-label="Pay the fee" href="/">',
        "    Fully decentralised",
        "  </Link>",
        ");",
      ].join("\n"),
  ...
  it("finds terms in a JSON file named explicitly", () => {
    project({ "copy.json": JSON.stringify({ checkout: { title: "Top up" } }, null, 2) });
    expect(found(scan(root, ["copy.json"]))).toEqual(["copy.json:3:Top up"]);
  });
  ...
  it("passes the app as it stands", () => {
    expect(found(scan(process.cwd(), ["src"]))).toEqual([]);
  });
});
