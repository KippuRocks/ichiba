...
#
# The package is not published to any registry. It is declarations only, and declares
# @trpc/server as a peer dependency, which this repository installs at the same version.
# The end-to-end tests run the kippu-api server from the same recorded commit
# (tools/test-api.sh), so the types Ichiba compiles against are the server it talks to.
set -euo pipefail
